...
from observatory.models.encoder_decoder import T5ModelWrapper
...
class TestT5EmbeddingInference(unittest.TestCase):
    def setUp(self):
        data_dir = os.path.join(
            os.path.abspath(os.path.dirname(__file__)),
            "sample_data/sotab",
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model_wrapper = T5ModelWrapper(model_name, device)
    ...
    def test_infer_column_embeddings(self):
        ...
        for batch_tables in self.sotab_dataloader:
            ...
            for batch_encoded_inputs, _ in encoded_inputs_dataloader:
                column_embeddings = self.model_wrapper.infer_embeddings(
                    batch_encoded_inputs,
                    use_decoder_last_hidden_state=True,
                    strategy="first",
                )
                ...
                # Use the encoder last hidden state
                column_embeddings = self.model_wrapper.infer_embeddings(
                    batch_encoded_inputs,
                    use_decoder_last_hidden_state=False,
                    strategy="first",
                )
